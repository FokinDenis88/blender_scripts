        ...
        if len(lists) > 1:  # Not in single if, because len(NonType)
            ...
            while i < len(lists):
                if len(lists[i]) != first_list_len:
                    if has_log:
                        print(are_lists_equal_length.__name__ + '(): lists len are not equal. ' + lists[i])
                    return False
                i += 1
    if has_log:
        print(are_lists_equal_length.__name__ + '(): lists length are equal')
    return True
    ...
    if is_not_none_or_empty(list_object):
        for object in list_object:
            if isinstance(object, bool):
                ...
                print(has_false_value.__name__ + 'values in list must be bool type')
                return None

        return False
    else:
        print(has_false_value.__name__ + 'list_object must not be empty or None')
        return None
...
def is_in_types(object, types_list):
    if object is not None and is_not_none_or_empty(types_list):
        ...
        print(is_in_types.__name__ + ': object and types_list must not be None or empty')
        return False
...
def disjunction_of_conditions(condition_list):
    if is_not_none_or_empty(condition_list):
        ...
        print(disjunction_of_conditions.__name__ + ': condition_list must not be None or empty')
        return None
...
def conjunction_of_conditions(condition_list):
    if is_not_none_or_empty(condition_list):
        ...
        print(conjunction_of_conditions.__name__ + ': condition_list must not be None or empty')
        return None